...
public class CharAtView extends BlockView {
	...
	public CharAtView(Observable model, Controller controller, IDEPanel p, LanguageModule lang) {
	...
	public static JPanel getRepresentative(LanguageModule lang){
		return new JPanel() {
...

			{
				setOpaque(false);
				setLayout(null);
				JLabel title = new JLabel(lang.getString("Bcharat"));
	...
	public void addComponents() {
		_title = new JLabel(_lang.getString("Bcharat"));
	...
	public void makeShapes() {
		...
		if(((CharAtController)getController()).getError())
	...
	public BlockView getBlockToUnsnap(int x, int y) {
		...
		if (_bodyL != null) {
			...
			if (rec == _bodyL){
				((CharAtController)getController()).removeContent(_bodyL);
				_bodyL = null;
		...
		if (rec == null && _bodyR != null) {
			...
			if (rec == _bodyR){
				((CharAtController)getController()).removeIndex(_bodyR);
				_bodyR = null;
				for (Component cinner : rec.getInnerComponents()) {
					removeFromPanel(cinner);
	...
	public Component add(Component comp) {
		if (_leftAdd) {
			...
			((CharAtController)getController()).addContent((BlockView) comp);
		} else {
			addViewRight((BlockView) comp);
			((CharAtController)getController()).addIndex((BlockView) comp);
		}
		return comp;
	}
	...
	private void setViewCorrect(BlockView view) {
		...
		_widthL = Math.max(view.getWidthSize(), _widthL);
	...
	private void setViewCorrectRight(BlockView view) {
		...
		_widthR = Math.max(view.getWidthSize(), _widthR);
	...
	protected boolean acceptBlock(Component comp) {
		return ((CharAtController)getController()).acceptSingle((BlockView) comp);
	}

	@Override
	public Controller defaultController(Observable model) {
		return new CharAtController(getModel());
	}
	...
	public void updateLanguage() {
		_title.setText(_lang.getString("Bcharat"));
	}
	...
	public String getHelpMenu() {
		return _lang.getString("BcharatHelp");
	}
}